"""
`LangmuirFreundlichS5(M₁, K₀₁, E₁, f₀₁, β₁, M₂, K₀₂, E₂, f₀₂, β₂, M₃, K₀₃, E₃, f₀₃, β₃, M₄, K₀₄, E₄, f₀₄, β₄, M₅, K₀₅, E₅, f₀₅, β₅)`

LangmuirFreundlichS5 <: IsothermModel
...
The Langmuir-Freundlich form of the isotherm for 5 sites is:

n = M₁ ⋅ K₁ ⋅ p₁^(f₀₁) ÷ (1 + K₁ ⋅ p₁^(f₀₁)) +  M₂ ⋅ K₂ ⋅ p₂^(f₀₂) ÷ (1 + K₂ ⋅ p₂^(f₀₂)) + M₃ ⋅ K₃ ⋅ p₃^(f₀₃) ÷ (1 + K₃ ⋅ p₃^(f₀₃)) + M₄ ⋅ K₄ ⋅ p₄^(f₀₄) ÷ (1 + K₄ ⋅ p₄^(f₀₄)) + M₅ ⋅ K₅ ⋅ p₅^(f₀₅) ÷ (1 + K₅ ⋅ p₅^(f₀₅))
...
"""
@with_metadata struct LangmuirFreundlichS5{T} <: IsothermModel{T}
    ...
    (f₀₅::T, (0.0, Inf), "Surface heterogeneity parameter 5 at T → ∞")
    (β₅::T, (-Inf, Inf), "Surface heterogeneity coefficient 5")
end


function sp_res(model::LangmuirFreundlichS5, p, T)
    M₁ = model.M₁
    K₀₁ = model.K₀₁
    E₁ = model.E₁
    M₂ = model.M₂
    ...
    K₅ = K₀₅*exp(-E₅/(Rgas(model)*T))
    f₅ = model.f₀₅ - model.β₅/T 
    return M₁*log1p(K₁*p^(f₁))/(f₁) + M₂*log1p(K₂*p^(f₂))/(f₂) + M₃*log1p(K₃*p^(f₃))/(f₃) + M₄*log1p(K₄*p^(f₄))/(f₄) + M₅*log1p(K₅*p^(f₅))/(f₅)
end

function loading(model::LangmuirFreundlichS5, p, T)
    M₁ = model.M₁
    K₀₁ = model.K₀₁
    E₁ = model.E₁
    M₂ = model.M₂
    ...
    Kpf₅ = K₅*p^f₅
    _1 = one(eltype(p))
    return M₁*Kpf₁/(_1 + Kpf₁) + M₂*Kpf₂/(_1 + Kpf₂) + M₃*Kpf₃/(_1 + Kpf₃) + M₄*Kpf₄/(_1 + Kpf₄) + M₅*Kpf₅/(_1 + Kpf₅)
end

saturated_loading(model::LangmuirFreundlichS5, T) = model.M₁ + model.M₂ + model.M₃ + model.M₄ + model.M₅

export LangmuirFreundlichS5